...
export class MailerService {
  ...
  async sendMailUsingTemplate(
    ...
    try {
      ...
      if (!template)
        throw new Error(`Kein Template mit Typ "${templateType}" gefunden`);
      ...
      if (this.#provider === 'brevo') {
        ...
        const fullName =
          `${placeholders['lastName'] ?? ''} ${placeholders['firstName'] ?? ''}`.trim() ||
      ...
      this.#logger
        ...
        .info(
          `Mail vom Typ "${templateType}" erfolgreich an ${toEmail} gesendet`,
        );
      ...
      this.#logger
        .withContext(traceContext)
        .error('Fehler beim Mailversand: %o', err);
      throw err;
    } finally {
      span.end();
    }
  ...
  #getFromUser(): string {
    switch (this.#provider) {
      case 'gmail':
        return process.env.MAIL_GMAIL_USER!;
      case 'outlook':
        return process.env.MAIL_OUTLOOK_USER!;
      case 'smtp':
        return process.env.MAIL_SMTP_USER!;
      case 'brevo':
        return process.env.MAIL_BREVO_USER!;
      default:
        throw new Error('Unbekannter Mailprovider');
    }
  }
